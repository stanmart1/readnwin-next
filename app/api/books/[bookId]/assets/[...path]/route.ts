...
export async function GET(
  ...
  try {
    ...
    const bookResult = await secureQuery(`
      SELECT b.id, bf.extraction_path
      FROM books b
    ...
    if (session?.user?.id) {
      const accessResult = await secureQuery(`
        SELECT 1 FROM books WHERE id = $1 AND (created_by = $2 OR price = 0)
        UNION
    ...
    const fullPath = path.join(storagePath, bookId.toString(), 'extracted', assetPath);
