...
export default function DashboardContent() {
  ...
  useEffect(() => {
    const sendWelcomeEmail = async () => {
      if (session?.user?.email && typeof window !== 'undefined' && !sessionStorage.getItem('welcome-email-sent')) {
        try {
              ...
              <nav className="flex space-x-4 sm:space-x-8 px-4 sm:px-6 overflow-x-auto scrollbar-hide" aria-label="Tabs">
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`py-3 sm:py-4 px-1 border-b-2 font-medium text-sm flex items-center space-x-1.5 sm:space-x-2 transition-colors duration-200 whitespace-nowrap ${
                      activeTab === tab.id
