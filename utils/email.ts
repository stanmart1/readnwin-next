...
async function getEmailGatewayConfig(): Promise<EmailGatewayConfig | null> {
  try {
    ...
    } else if (activeGateway === 'smtp' && gatewayConfig.smtpHost && gatewayConfig.smtpUsername && gatewayConfig.smtpPassword) {
      gatewayConfig.isActive = true;
    }
...
export const getEmailVerificationTemplate = (userName: string, verificationToken: string): EmailTemplate => ({
...
export const getPasswordResetEmailTemplate = (resetToken: string, userName: string): EmailTemplate => ({
...
export const getOrderConfirmationEmailTemplate = (orderDetails: any, userName: string): EmailTemplate => ({
...
export const getEmailConfirmationTemplate = (userName: string): EmailTemplate => ({
...
export const sendWelcomeEmail = async (userEmail: string, userName: string, userId?: number) => {
  try {
...
export const sendPasswordResetEmail = async (userEmail: string, resetToken: string, userName: string) => {
  try {
    // Use the new function-based email system
    ...
    const result = await sendFunctionEmail(userEmail, 'password-reset', variables);
    return result;
  } catch (error) {
...
export const sendOrderShippedEmail = async (userEmail: string, orderDetails: any, userName: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      userName: userName,
      orderNumber: orderDetails.orderId || orderDetails.orderNumber,
      trackingNumber: orderDetails.trackingNumber || 'TRK' + Math.random().toString(36).substr(2, 9).toUpperCase(),
      estimatedDelivery: orderDetails.estimatedDelivery || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString()
    };

    const result = await sendFunctionEmail(userEmail, 'order-shipped', variables);
    return result;
  } catch (error) {
...
export const sendEmailVerification = async (userEmail: string, verificationToken: string, userName: string) => {
  try {
    // Use the new function-based email system
    ...
    const result = await sendFunctionEmail(userEmail, 'account-verification', variables);
    return result;
  } catch (error) {
...
export const sendEmailConfirmation = async (userEmail: string, userName: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      userName: userName,
    ...
    const result = await sendFunctionEmail(userEmail, 'email-confirmation', variables);
    return result;
  } catch (error) {
...
export const sendOrderStatusUpdateEmail = async (userEmail: string, orderDetails: any, userName: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      userName: userName,
      ...
      statusDescription: orderDetails.statusDescription || getStatusDescription(orderDetails.status)
    };

    const result = await sendFunctionEmail(userEmail, 'order-status-update', variables);
    return result;
  } catch (error) {
...
export const sendPaymentConfirmationEmail = async (userEmail: string, orderDetails: any, userName: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      userName: userName,
    ...
    const result = await sendFunctionEmail(userEmail, 'payment-confirmation', variables);
    return result;
  } catch (error) {
...
export const sendShippingNotificationEmail = async (userEmail: string, orderDetails: any, userName: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      userName: userName,
    ...
    const result = await sendFunctionEmail(userEmail, 'shipping-notification', variables);
    return result;
  } catch (error) {
...
export const sendPasswordChangedEmail = async (userEmail: string, userName: string, changedAt: string, ipAddress: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      userName: userName,
    ...
    const result = await sendFunctionEmail(userEmail, 'password-changed', variables);
    return result;
  } catch (error) {
...
export const sendLoginAlertEmail = async (userEmail: string, userName: string, loginTime: string, ipAddress: string, deviceInfo: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      userName: userName,
...
export const sendAccountDeactivationEmail = async (userEmail: string, userName: string, deactivationReason: string, reactivationUrl: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      userName: userName,
    ...
    const result = await sendFunctionEmail(userEmail, 'account-deactivation', variables);
    return result;
  } catch (error) {
...
export const sendNewsletterSubscriptionEmail = async (userEmail: string, userName: string, subscriptionType: string, unsubscribeUrl: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      userName: userName,
    ...
    const result = await sendFunctionEmail(userEmail, 'newsletter-subscription', variables);
    return result;
  } catch (error) {
...
export const sendPromotionalOfferEmail = async (userEmail: string, userName: string, offerTitle: string, offerDescription: string, expiryDate: string, discountCode: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      userName: userName,
    ...
    const result = await sendFunctionEmail(userEmail, 'promotional-offer', variables);
    return result;
  } catch (error) {
...
export const sendSystemMaintenanceEmail = async (userEmail: string, maintenanceType: string, startTime: string, endTime: string, affectedServices: string) => {
  try {
    // Use the new function-based email system
    const variables = {
    ...
    const result = await sendFunctionEmail(userEmail, 'system-maintenance', variables);
    return result;
  } catch (error) {
...
export const sendSecurityAlertEmail = async (userEmail: string, alertType: string, severity: string, description: string, actionRequired: string) => {
  try {
    // Use the new function-based email system
    const variables = {
      alertType: alertType,
    ...
    const result = await sendFunctionEmail(userEmail, 'security-alert', variables);
    return result;
  } catch (error) {
...
const getStatusDescription = (status: string): string => {
  const descriptions: { [key: string]: string } = {
    ...
    'payment_processing': 'We are processing your payment. This usually takes a few minutes.',
    ...
    'refunded': 'Your order has been refunded and the amount will be returned to your payment method.'
  };
...
export const sendTemplateEmail = async (to: string, templateSlug: string, variables: Record<string, any>) => {
  try {
    const renderedTemplate = await emailTemplateService.renderTemplate(templateSlug, variables);
    
    if (!renderedTemplate) {
      throw new Error(`Template '${templateSlug}' not found or inactive`);
    }

    return await sendEmail(to, renderedTemplate.subject, renderedTemplate.html, renderedTemplate.text);
  } catch (error) {
...
export const sendEmail = async (to: string, subject: string, html: string, text?: string) => {
  try {
    ...
    throw new Error('No email gateway configured. Please configure an email gateway in the admin settings.');
  } catch (error) {
...
export const sendFunctionEmail = async (to: string, functionSlug: string, variables: Record<string, any>) => {
  try {
    const template = await emailTemplateService.getTemplateForFunction(functionSlug, variables);
    
    if (!template) {
      throw new Error(`No active template assigned to function: ${functionSlug}`);
    }
